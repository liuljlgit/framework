...
import com.cloud.frame.frameauth.cache.IComRoleCache;
...
/**
 * IComRoleCache cache实现类
 * @author lijun
 */
@Slf4j
public class ComRoleCacheImpl extends BaseServiceImpl<ComRole> implements IComRoleCache {

    private final static Long DEFAULT_EXPIRE_TIMES = 30L;
    private final static TimeUnit DEFAULT_EXPIRE_TIMEUNIT = TimeUnit.MINUTES;
    private final static String CLS_NAME = ComRole.class.getSimpleName();
    ...
    @Override
    public ComRole cacheSelectById(Serializable id, String... nullErrMsg) {
        String entityKey = CLS_NAME.concat(":").concat(String.valueOf(id));
        ParserConfig.getGlobalInstance().setAutoTypeSupport(true);
        ComRole cacheVal = (ComRole)redisTemplate.opsForValue().get(entityKey);
        if(Objects.nonNull(cacheVal)){
            return cacheVal;
        }
        ComRole dbVal = super.selectById(id, nullErrMsg);
    ...
    @Override
    public ComRole cacheSelectOne(ComRole query, String... nullErrMsg) {
        query.setPage(1);
        query.setPageSize(1);
        String queryKey = QUERY_KEY.concat("SelectOne:")
                .concat(QueryKeyUtil.getQueryKey(query, Boolean.TRUE));
        ParserConfig.getGlobalInstance().setAutoTypeSupport(true);
        ComRole cacheVal = (ComRole)redisTemplate.opsForValue().get(queryKey);
        if(Objects.nonNull(cacheVal)){
            return cacheVal;
        }
        ComRole dbVal = super.selectOne(query, nullErrMsg);
        if(Objects.nonNull(dbVal)){
    ...
    public List<ComRole> cacheSelectList(ComRole query, String... emptyErrMsg) {
        ...
        if(!CollectionUtils.isEmpty(cacheKeys)){
            ...
            if(Objects.nonNull(cacheValues) && cacheKeys.size() == cacheValues.size()){
                return JSONArray.parseArray(JSON.toJSONString(cacheValues), ComRole.class);
            }
        }
        List<ComRole> dbValues = super.selectList(query, emptyErrMsg);
        List<String> dbKeys = Lists.newArrayList();
        dbValues.forEach(e -> {
            String entityKey = CLS_NAME.concat(":").concat(String.valueOf(e.getRoleId()));
    ...
    @Override
    @SuppressWarnings("unchecked")
    public List<ComRole> cacheSelectList(ComRole query, List<String> fieldList, String... emptyErrMsg) {
        ...
        List<ComRole> dbValues = super.selectList(query, fieldList, emptyErrMsg);
    ...
    @Override
    public List<ComRole> cacheSelectBatchIds(Collection<? extends Serializable> list, String... emptyErrMsg) {
        ...
        if(!CollectionUtils.isEmpty(queryKeys)){
            ...
            if(Objects.nonNull(cacheValues)){
                List<ComRole> cacheEntitys = JSONArray.parseArray(JSON.toJSONString(cacheValues), ComRole.class);
                if(queryKeys.size() == cacheEntitys.size()){
                    return cacheEntitys;
                } else {
                    List<String> cacheKeys = cacheEntitys.stream()
                        .map(e -> CLS_NAME.concat(":").concat(String.valueOf(e.getRoleId())))
                        .collect(Collectors.toList());
                    queryKeys.removeAll(cacheKeys);
                    List<ComRole> dbValues = super.selectBatchIds(queryKeys, emptyErrMsg);
                    if(!CollectionUtils.isEmpty(dbValues)){
                ...
                List<ComRole> dbValues = super.selectBatchIds(list, emptyErrMsg);
                dbValues.forEach(e -> {
                    String entityKey = CLS_NAME.concat(":").concat(String.valueOf(e.getRoleId()));
                    redisTemplate.opsForValue().set(entityKey,e);
                    redisTemplate.expire(entityKey,DEFAULT_EXPIRE_TIMES,DEFAULT_EXPIRE_TIMEUNIT);
    ...
    @Override
    @SuppressWarnings("unchecked")
    public PageBean<ComRole> cacheSelectPage(ComRole query) {
        ...
        PageBean<ComRole> cachePageBean = (PageBean<ComRole>)redisTemplate.opsForValue().get(pageBeanKey);
        ...
        List<Object> cacheValues = redisTemplate.opsForValue().multiGet(cahcePageKeys);
            ...
            List<ComRole> list = JSONArray.parseArray(JSON.toJSONString(cacheValues), ComRole.class);
                cachePageBean.setList(list);
                return cachePageBean;
            }
        }
        PageBean<ComRole> dbPageBean = super.selectPage(query);
    ...
    @Override
    @SuppressWarnings("unchecked")
    public PageBean<ComRole> cacheSelectPage(ComRole query, List<String> fieldList) {
        ...
        PageBean<ComRole> cachePageBean = (PageBean<ComRole>)redisTemplate.opsForValue().get(queryKey);
        if(Objects.nonNull(cachePageBean)){
            return cachePageBean;
        }
        PageBean<ComRole> dbPageBean = super.selectPage(query, fieldList);
    ...
    @Override
    public Long cacheSelectCount(ComRole query) {
        ...
        if(Objects.nonNull(cacheCount)){
            log.info("Cache Class = {}, Entity = {} Count",ComRole.class.getName(),query);
            return cacheCount;
        }
    ...
    @Override
    public int update(ComRole entity, Update... args) {
    ...
    @Override
    public int updateByMap(ComRole oEntity, Map<String, Object> updateMap) {
    ...
    @Override
    public void updateBatch(List<ComRole> list, Update... args) {
        super.updateBatch(list, args);
        clearAllCacheData();
    }

    @Override
    public int add(ComRole entity) {
    ...
    @Override
    public void addBatch(List<ComRole> list) {
        super.addBatch(list);
        clearAllCacheData();
    }

    @Override
    public void addBatch(List<ComRole> list, int batchSize) {
        super.addBatch(list, batchSize);
        clearAllCacheData();
    }

    @Override
    public void delete(ComRole entity) {
        super.delete(entity);
        clearAllCacheData();
    }
    ...
    @Override
    public void save(ComRole comRole, Update... args) {
        super.save(comRole, args);
        clearAllCacheData();
    }

    @Override
    public void saveBatch(List<ComRole> list, Update... args) {
    ...
    private void clearAllCacheData(){
        try {
            Set<String> keys = redisTemplate.keys(ComRole.class.getSimpleName() + "*");
