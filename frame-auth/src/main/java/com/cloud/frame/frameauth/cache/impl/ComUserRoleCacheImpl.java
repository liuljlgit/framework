...
import com.cloud.frame.frameauth.cache.IComUserRoleCache;
...
/**
 * IComUserRoleCache cache实现类
 * @author lijun
 */
@Slf4j
public class ComUserRoleCacheImpl extends BaseServiceImpl<ComUserRole> implements IComUserRoleCache {

    private final static Long DEFAULT_EXPIRE_TIMES = 30L;
    private final static TimeUnit DEFAULT_EXPIRE_TIMEUNIT = TimeUnit.MINUTES;
    private final static String CLS_NAME = ComUserRole.class.getSimpleName();
    ...
    @Override
    public ComUserRole cacheSelectById(Serializable id, String... nullErrMsg) {
        String entityKey = CLS_NAME.concat(":").concat(String.valueOf(id));
        ParserConfig.getGlobalInstance().setAutoTypeSupport(true);
        ComUserRole cacheVal = (ComUserRole)redisTemplate.opsForValue().get(entityKey);
        if(Objects.nonNull(cacheVal)){
            return cacheVal;
        }
        ComUserRole dbVal = super.selectById(id, nullErrMsg);
    ...
    @Override
    public ComUserRole cacheSelectOne(ComUserRole query, String... nullErrMsg) {
        query.setPage(1);
        query.setPageSize(1);
        String queryKey = QUERY_KEY.concat("SelectOne:")
                .concat(QueryKeyUtil.getQueryKey(query, Boolean.TRUE));
        ParserConfig.getGlobalInstance().setAutoTypeSupport(true);
        ComUserRole cacheVal = (ComUserRole)redisTemplate.opsForValue().get(queryKey);
        if(Objects.nonNull(cacheVal)){
            return cacheVal;
        }
        ComUserRole dbVal = super.selectOne(query, nullErrMsg);
        if(Objects.nonNull(dbVal)){
    ...
    public List<ComUserRole> cacheSelectList(ComUserRole query, String... emptyErrMsg) {
        ...
        if(!CollectionUtils.isEmpty(cacheKeys)){
            ...
            if(Objects.nonNull(cacheValues) && cacheKeys.size() == cacheValues.size()){
                return JSONArray.parseArray(JSON.toJSONString(cacheValues), ComUserRole.class);
            }
        }
        List<ComUserRole> dbValues = super.selectList(query, emptyErrMsg);
        List<String> dbKeys = Lists.newArrayList();
        dbValues.forEach(e -> {
            String entityKey = CLS_NAME.concat(":").concat(String.valueOf(e.getUrId()));
    ...
    @Override
    @SuppressWarnings("unchecked")
    public List<ComUserRole> cacheSelectList(ComUserRole query, List<String> fieldList, String... emptyErrMsg) {
        ...
        List<ComUserRole> dbValues = super.selectList(query, fieldList, emptyErrMsg);
    ...
    @Override
    public List<ComUserRole> cacheSelectBatchIds(Collection<? extends Serializable> list, String... emptyErrMsg) {
        ...
        if(!CollectionUtils.isEmpty(queryKeys)){
            ...
            if(Objects.nonNull(cacheValues)){
                List<ComUserRole> cacheEntitys = JSONArray.parseArray(JSON.toJSONString(cacheValues), ComUserRole.class);
                if(queryKeys.size() == cacheEntitys.size()){
                    return cacheEntitys;
                } else {
                    List<String> cacheKeys = cacheEntitys.stream()
                        .map(e -> CLS_NAME.concat(":").concat(String.valueOf(e.getUrId())))
                        .collect(Collectors.toList());
                    queryKeys.removeAll(cacheKeys);
                    List<ComUserRole> dbValues = super.selectBatchIds(queryKeys, emptyErrMsg);
                    if(!CollectionUtils.isEmpty(dbValues)){
                ...
                List<ComUserRole> dbValues = super.selectBatchIds(list, emptyErrMsg);
                dbValues.forEach(e -> {
                    String entityKey = CLS_NAME.concat(":").concat(String.valueOf(e.getUrId()));
                    redisTemplate.opsForValue().set(entityKey,e);
                    redisTemplate.expire(entityKey,DEFAULT_EXPIRE_TIMES,DEFAULT_EXPIRE_TIMEUNIT);
    ...
    @Override
    @SuppressWarnings("unchecked")
    public PageBean<ComUserRole> cacheSelectPage(ComUserRole query) {
        ...
        PageBean<ComUserRole> cachePageBean = (PageBean<ComUserRole>)redisTemplate.opsForValue().get(pageBeanKey);
        ...
        List<Object> cacheValues = redisTemplate.opsForValue().multiGet(cahcePageKeys);
            ...
            List<ComUserRole> list = JSONArray.parseArray(JSON.toJSONString(cacheValues), ComUserRole.class);
                cachePageBean.setList(list);
                return cachePageBean;
            }
        }
        PageBean<ComUserRole> dbPageBean = super.selectPage(query);
    ...
    @Override
    @SuppressWarnings("unchecked")
    public PageBean<ComUserRole> cacheSelectPage(ComUserRole query, List<String> fieldList) {
        ...
        PageBean<ComUserRole> cachePageBean = (PageBean<ComUserRole>)redisTemplate.opsForValue().get(queryKey);
        if(Objects.nonNull(cachePageBean)){
            return cachePageBean;
        }
        PageBean<ComUserRole> dbPageBean = super.selectPage(query, fieldList);
    ...
    @Override
    public Long cacheSelectCount(ComUserRole query) {
        ...
        if(Objects.nonNull(cacheCount)){
            log.info("Cache Class = {}, Entity = {} Count",ComUserRole.class.getName(),query);
            return cacheCount;
        }
    ...
    @Override
    public int update(ComUserRole entity, Update... args) {
    ...
    @Override
    public int updateByMap(ComUserRole oEntity, Map<String, Object> updateMap) {
    ...
    @Override
    public void updateBatch(List<ComUserRole> list, Update... args) {
        super.updateBatch(list, args);
        clearAllCacheData();
    }

    @Override
    public int add(ComUserRole entity) {
    ...
    @Override
    public void addBatch(List<ComUserRole> list) {
        super.addBatch(list);
        clearAllCacheData();
    }

    @Override
    public void addBatch(List<ComUserRole> list, int batchSize) {
        super.addBatch(list, batchSize);
        clearAllCacheData();
    }

    @Override
    public void delete(ComUserRole entity) {
        super.delete(entity);
        clearAllCacheData();
    }
    ...
    @Override
    public void save(ComUserRole comUserRole, Update... args) {
        super.save(comUserRole, args);
        clearAllCacheData();
    }

    @Override
    public void saveBatch(List<ComUserRole> list, Update... args) {
    ...
    private void clearAllCacheData(){
        try {
            Set<String> keys = redisTemplate.keys(ComUserRole.class.getSimpleName() + "*");
