...
import com.cloud.frame.frameauth.cache.IComAuthorityCache;
...
/**
 * IComAuthorityCache cache实现类
 * @author lijun
 */
@Slf4j
public class ComAuthorityCacheImpl extends BaseServiceImpl<ComAuthority> implements IComAuthorityCache {

    private final static Long DEFAULT_EXPIRE_TIMES = 30L;
    private final static TimeUnit DEFAULT_EXPIRE_TIMEUNIT = TimeUnit.MINUTES;
    private final static String CLS_NAME = ComAuthority.class.getSimpleName();
    ...
    @Override
    public ComAuthority cacheSelectById(Serializable id, String... nullErrMsg) {
        String entityKey = CLS_NAME.concat(":").concat(String.valueOf(id));
        ParserConfig.getGlobalInstance().setAutoTypeSupport(true);
        ComAuthority cacheVal = (ComAuthority)redisTemplate.opsForValue().get(entityKey);
        if(Objects.nonNull(cacheVal)){
            return cacheVal;
        }
        ComAuthority dbVal = super.selectById(id, nullErrMsg);
    ...
    @Override
    public ComAuthority cacheSelectOne(ComAuthority query, String... nullErrMsg) {
        query.setPage(1);
        query.setPageSize(1);
        String queryKey = QUERY_KEY.concat("SelectOne:")
                .concat(QueryKeyUtil.getQueryKey(query, Boolean.TRUE));
        ParserConfig.getGlobalInstance().setAutoTypeSupport(true);
        ComAuthority cacheVal = (ComAuthority)redisTemplate.opsForValue().get(queryKey);
        if(Objects.nonNull(cacheVal)){
            return cacheVal;
        }
        ComAuthority dbVal = super.selectOne(query, nullErrMsg);
        if(Objects.nonNull(dbVal)){
    ...
    public List<ComAuthority> cacheSelectList(ComAuthority query, String... emptyErrMsg) {
        ...
        if(!CollectionUtils.isEmpty(cacheKeys)){
            ...
            if(Objects.nonNull(cacheValues) && cacheKeys.size() == cacheValues.size()){
                return JSONArray.parseArray(JSON.toJSONString(cacheValues), ComAuthority.class);
            }
        }
        List<ComAuthority> dbValues = super.selectList(query, emptyErrMsg);
        List<String> dbKeys = Lists.newArrayList();
        dbValues.forEach(e -> {
            String entityKey = CLS_NAME.concat(":").concat(String.valueOf(e.getAuthId()));
    ...
    @Override
    @SuppressWarnings("unchecked")
    public List<ComAuthority> cacheSelectList(ComAuthority query, List<String> fieldList, String... emptyErrMsg) {
        ...
        List<ComAuthority> dbValues = super.selectList(query, fieldList, emptyErrMsg);
    ...
    @Override
    public List<ComAuthority> cacheSelectBatchIds(Collection<? extends Serializable> list, String... emptyErrMsg) {
        ...
        if(!CollectionUtils.isEmpty(queryKeys)){
            ...
            if(Objects.nonNull(cacheValues)){
                List<ComAuthority> cacheEntitys = JSONArray.parseArray(JSON.toJSONString(cacheValues), ComAuthority.class);
                if(queryKeys.size() == cacheEntitys.size()){
                    return cacheEntitys;
                } else {
                    List<String> cacheKeys = cacheEntitys.stream()
                        .map(e -> CLS_NAME.concat(":").concat(String.valueOf(e.getAuthId())))
                        .collect(Collectors.toList());
                    queryKeys.removeAll(cacheKeys);
                    List<ComAuthority> dbValues = super.selectBatchIds(queryKeys, emptyErrMsg);
                    if(!CollectionUtils.isEmpty(dbValues)){
                ...
                List<ComAuthority> dbValues = super.selectBatchIds(list, emptyErrMsg);
                dbValues.forEach(e -> {
                    String entityKey = CLS_NAME.concat(":").concat(String.valueOf(e.getAuthId()));
                    redisTemplate.opsForValue().set(entityKey,e);
                    redisTemplate.expire(entityKey,DEFAULT_EXPIRE_TIMES,DEFAULT_EXPIRE_TIMEUNIT);
    ...
    @Override
    @SuppressWarnings("unchecked")
    public PageBean<ComAuthority> cacheSelectPage(ComAuthority query) {
        ...
        PageBean<ComAuthority> cachePageBean = (PageBean<ComAuthority>)redisTemplate.opsForValue().get(pageBeanKey);
        ...
        List<Object> cacheValues = redisTemplate.opsForValue().multiGet(cahcePageKeys);
            ...
            List<ComAuthority> list = JSONArray.parseArray(JSON.toJSONString(cacheValues), ComAuthority.class);
                cachePageBean.setList(list);
                return cachePageBean;
            }
        }
        PageBean<ComAuthority> dbPageBean = super.selectPage(query);
    ...
    @Override
    @SuppressWarnings("unchecked")
    public PageBean<ComAuthority> cacheSelectPage(ComAuthority query, List<String> fieldList) {
        ...
        PageBean<ComAuthority> cachePageBean = (PageBean<ComAuthority>)redisTemplate.opsForValue().get(queryKey);
        if(Objects.nonNull(cachePageBean)){
            return cachePageBean;
        }
        PageBean<ComAuthority> dbPageBean = super.selectPage(query, fieldList);
    ...
    @Override
    public Long cacheSelectCount(ComAuthority query) {
        ...
        if(Objects.nonNull(cacheCount)){
            log.info("Cache Class = {}, Entity = {} Count",ComAuthority.class.getName(),query);
            return cacheCount;
        }
    ...
    @Override
    public int update(ComAuthority entity, Update... args) {
    ...
    @Override
    public int updateByMap(ComAuthority oEntity, Map<String, Object> updateMap) {
    ...
    @Override
    public void updateBatch(List<ComAuthority> list, Update... args) {
        super.updateBatch(list, args);
        clearAllCacheData();
    }

    @Override
    public int add(ComAuthority entity) {
    ...
    @Override
    public void addBatch(List<ComAuthority> list) {
        super.addBatch(list);
        clearAllCacheData();
    }

    @Override
    public void addBatch(List<ComAuthority> list, int batchSize) {
        super.addBatch(list, batchSize);
        clearAllCacheData();
    }

    @Override
    public void delete(ComAuthority entity) {
        super.delete(entity);
        clearAllCacheData();
    }
    ...
    @Override
    public void save(ComAuthority comAuthority, Update... args) {
        super.save(comAuthority, args);
        clearAllCacheData();
    }

    @Override
    public void saveBatch(List<ComAuthority> list, Update... args) {
    ...
    private void clearAllCacheData(){
        try {
            Set<String> keys = redisTemplate.keys(ComAuthority.class.getSimpleName() + "*");
