...
import com.cloud.frame.frameauth.cache.IComFormCache;
...
/**
 * IComFormCache cache实现类
 * @author lijun
 */
@Slf4j
public class ComFormCacheImpl extends BaseServiceImpl<ComForm> implements IComFormCache {

    private final static Long DEFAULT_EXPIRE_TIMES = 30L;
    private final static TimeUnit DEFAULT_EXPIRE_TIMEUNIT = TimeUnit.MINUTES;
    private final static String CLS_NAME = ComForm.class.getSimpleName();
    ...
    @Override
    public ComForm cacheSelectById(Serializable id, String... nullErrMsg) {
        String entityKey = CLS_NAME.concat(":").concat(String.valueOf(id));
        ParserConfig.getGlobalInstance().setAutoTypeSupport(true);
        ComForm cacheVal = (ComForm)redisTemplate.opsForValue().get(entityKey);
        if(Objects.nonNull(cacheVal)){
            return cacheVal;
        }
        ComForm dbVal = super.selectById(id, nullErrMsg);
    ...
    @Override
    public ComForm cacheSelectOne(ComForm query, String... nullErrMsg) {
        query.setPage(1);
        query.setPageSize(1);
        String queryKey = QUERY_KEY.concat("SelectOne:")
                .concat(QueryKeyUtil.getQueryKey(query, Boolean.TRUE));
        ParserConfig.getGlobalInstance().setAutoTypeSupport(true);
        ComForm cacheVal = (ComForm)redisTemplate.opsForValue().get(queryKey);
        if(Objects.nonNull(cacheVal)){
            return cacheVal;
        }
        ComForm dbVal = super.selectOne(query, nullErrMsg);
        if(Objects.nonNull(dbVal)){
    ...
    public List<ComForm> cacheSelectList(ComForm query, String... emptyErrMsg) {
        ...
        if(!CollectionUtils.isEmpty(cacheKeys)){
            ...
            if(Objects.nonNull(cacheValues) && cacheKeys.size() == cacheValues.size()){
                return JSONArray.parseArray(JSON.toJSONString(cacheValues), ComForm.class);
            }
        }
        List<ComForm> dbValues = super.selectList(query, emptyErrMsg);
        List<String> dbKeys = Lists.newArrayList();
        dbValues.forEach(e -> {
            String entityKey = CLS_NAME.concat(":").concat(String.valueOf(e.getFormId()));
    ...
    @Override
    @SuppressWarnings("unchecked")
    public List<ComForm> cacheSelectList(ComForm query, List<String> fieldList, String... emptyErrMsg) {
        ...
        List<ComForm> dbValues = super.selectList(query, fieldList, emptyErrMsg);
    ...
    @Override
    public List<ComForm> cacheSelectBatchIds(Collection<? extends Serializable> list, String... emptyErrMsg) {
        ...
        if(!CollectionUtils.isEmpty(queryKeys)){
            ...
            if(Objects.nonNull(cacheValues)){
                List<ComForm> cacheEntitys = JSONArray.parseArray(JSON.toJSONString(cacheValues), ComForm.class);
                if(queryKeys.size() == cacheEntitys.size()){
                    return cacheEntitys;
                } else {
                    List<String> cacheKeys = cacheEntitys.stream()
                        .map(e -> CLS_NAME.concat(":").concat(String.valueOf(e.getFormId())))
                        .collect(Collectors.toList());
                    queryKeys.removeAll(cacheKeys);
                    List<ComForm> dbValues = super.selectBatchIds(queryKeys, emptyErrMsg);
                    if(!CollectionUtils.isEmpty(dbValues)){
                ...
                List<ComForm> dbValues = super.selectBatchIds(list, emptyErrMsg);
                dbValues.forEach(e -> {
                    String entityKey = CLS_NAME.concat(":").concat(String.valueOf(e.getFormId()));
                    redisTemplate.opsForValue().set(entityKey,e);
                    redisTemplate.expire(entityKey,DEFAULT_EXPIRE_TIMES,DEFAULT_EXPIRE_TIMEUNIT);
    ...
    @Override
    @SuppressWarnings("unchecked")
    public PageBean<ComForm> cacheSelectPage(ComForm query) {
        ...
        PageBean<ComForm> cachePageBean = (PageBean<ComForm>)redisTemplate.opsForValue().get(pageBeanKey);
        ...
        List<Object> cacheValues = redisTemplate.opsForValue().multiGet(cahcePageKeys);
            ...
            List<ComForm> list = JSONArray.parseArray(JSON.toJSONString(cacheValues), ComForm.class);
                cachePageBean.setList(list);
                return cachePageBean;
            }
        }
        PageBean<ComForm> dbPageBean = super.selectPage(query);
    ...
    @Override
    @SuppressWarnings("unchecked")
    public PageBean<ComForm> cacheSelectPage(ComForm query, List<String> fieldList) {
        ...
        PageBean<ComForm> cachePageBean = (PageBean<ComForm>)redisTemplate.opsForValue().get(queryKey);
        if(Objects.nonNull(cachePageBean)){
            return cachePageBean;
        }
        PageBean<ComForm> dbPageBean = super.selectPage(query, fieldList);
    ...
    @Override
    public Long cacheSelectCount(ComForm query) {
        ...
        if(Objects.nonNull(cacheCount)){
            log.info("Cache Class = {}, Entity = {} Count",ComForm.class.getName(),query);
            return cacheCount;
        }
    ...
    @Override
    public int update(ComForm entity, Update... args) {
    ...
    @Override
    public int updateByMap(ComForm oEntity, Map<String, Object> updateMap) {
    ...
    @Override
    public void updateBatch(List<ComForm> list, Update... args) {
        super.updateBatch(list, args);
        clearAllCacheData();
    }

    @Override
    public int add(ComForm entity) {
    ...
    @Override
    public void addBatch(List<ComForm> list) {
        super.addBatch(list);
        clearAllCacheData();
    }

    @Override
    public void addBatch(List<ComForm> list, int batchSize) {
        super.addBatch(list, batchSize);
        clearAllCacheData();
    }

    @Override
    public void delete(ComForm entity) {
        super.delete(entity);
        clearAllCacheData();
    }
    ...
    @Override
    public void save(ComForm comForm, Update... args) {
        super.save(comForm, args);
        clearAllCacheData();
    }

    @Override
    public void saveBatch(List<ComForm> list, Update... args) {
    ...
    private void clearAllCacheData(){
        try {
            Set<String> keys = redisTemplate.keys(ComForm.class.getSimpleName() + "*");
